...
inputs:
    -lat_min: the minimum latitude of the band
    -lat_max: the maximum latitude of the band
    -site_codes: list of all the site codes
    -site_info_file: the csv file containing the site information 
    
returns:
    -new_sites: a revised list of site codes that eliminates all outside of the band
    -lat_str: a naming string for use specifying the latitude band used
'''
def filter_site_codes(lat_min, lat_max, site_codes, site_info_file):
...
inputs: 
    ...
    -weight_std_do: use the inverse square of the stdev as the weight for the drift calculation
    -lat_str: a naming string for use specifying the latitude band used

returns:
    -drift_arr: a np array with the drift at each of the 10 levels (200:900 hPa, surface, column), only valid for column
    -drift_unc_arr: a np array with the standard error of the drift at each of the 10 levels (200:900 hPa, surface, column), only valid for column
    -drift_perc_arr: a np array with the % drift at each of the 10 levels (200:900 hPa, surface, column)
    -drift_perc_unc_arr: a np array with the standard error of the % drift at each of the 10 levels (200:900 hPa, surface, column)
    -p_val_corr_arr: a np array with the p-value calculated for the data's correlation at each of the 10 levels (200:900 hPa, surface, column)
    -p_val_slope_arr: a np array with the p_value calculated for the data's slope at each of the 10 levels (200:900 hPa, surface, column)

outputs:
    -10 panel plot of the correlation 
    -.dat file with Latex formated table of drift
...
def plot_biasdrift_main(mop_prod, file_loc, file_str, site_codes, site_info_file, ngd_min, plot_type, weight_std_do, lat_str, igd_flag): #plot 9 levels plus total column
    ...
    #filter the site codes to the desired latitude band; currently not functional
    #site_codes = filter_site_codes(lat_min, lat_max, site_codes) 
    ...
    for i_lev in range(nlevs):
        ...
        for i_site in range(n_sites):
            ...
            for i_file in range(nvalfiles):
                ...
                #extract time info from filename if first level
                date_str = site_filelist[i_file].split(site_codes[i_site]+'.')[1].split('.')[0]
                yr_str = int(date_str[:4])
                mon_str = int(date_str[4:6])
                day_str = int(date_str[6:])
                
                #create year fraction for this file
                year_frac[i_file] = int(yr_str) + ((datetime(yr_str, mon_str, day_str)-datetime(yr_str, 1, 1)).days)/((datetime(yr_str+1, 1, 1)-datetime(yr_str, 1, 1)).days)
...
if __name__ == "__main__":
    ...
    #select whether or not to use weights (assigned as the inverse stdev of the data) in fitting the data
    weight_std_do = 'yes'
    ...
    #running paramaters required for filtering the sites to a specific latitude band.
    lat_min = -90
    lat_max = 90
    
    #function to trim the site_codes to only those within a desired band
    site_codes, lat_str = filter_site_codes(lat_min, lat_max, site_codes, site_info_file)
